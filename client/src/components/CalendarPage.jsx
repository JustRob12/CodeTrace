...
const CalendarPage = () => {
    ...
    const handleSaveEvent = async () => {
        if (eventTitle && eventStart && eventEnd) {
            ...
            try {
                const response = await axios.post('http://localhost:5000/api/auth/event', newEvent);
                const addedEvent = {
    ...
    const renderEventForm = () => (
        <div className="flex flex-col items-start space-y-4 bg-white p-6 rounded-lg shadow-md">
            ...
            <button
                ...
                className="w-full bg-blue-500 text-white p-2 rounded-md hover:bg-white-600 transition duration-300"
            >
                Save Event
            </button>
        </div>
                ...
                <div className="modal-content p-4">
                    ...
                    {selectedEvent ? (
                        <div className="event-details">
                            ...
                            <div className="button-container mt-4 flex space-x-2">
                                <button className="delete-button bg-red-500 text-white p-2 rounded-md hover:bg-red-600" onClick={() => handleDeleteEvent(selectedEvent.id)}>Delete Event</button>
                                <button className="close-button bg-gray-300 text-black p-2 rounded-md hover:bg-gray-400" onClick={closeModal}>Close</button>
                            </div>
                        </div>
                    ) : (
                        <p>Loading event details...</p> // Handle case when event is not available yet
                    )}
                </div>
            </Modal>
        </div>
